...
client = OpenAI(api_key = "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",)
class Personaje:
...
def seleccionar_personaje():
    '''AI_prompt = "Dame una frase de inicio para un juego de roles"
    response = openai.Completion.create(engine = 'gpt-3.5-turbo-1106', prompt = AI_prompt, n=1, top = None,temperature = 0.7)
    AI_response = response.choices[0].text.strip()
    frase_inicio = AI_response
    print(frase_inicio)'''
    messages = [{"role": "user", "content":
                 f"Genera un mensaje motivacional de no más de 50 caracteres dando la bienvenida a un juego de roles"}]
    ...
    while personaje.salud > 0:
        ...
        if personaje.salud <= 0:
            ...
            message2 = [{"role": "user", "content":
                 f"Genera un mensaje motivacional de no más de 50 caracteres consolando al perdedor"}]
            chat = client.chat.completions.create(
            model = "gpt-3.5-turbo", messages = message2
            )
